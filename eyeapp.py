...
if "tab_index" not in st.session_state:
    st.session_state.tab_index = 0  # 0: Pre-Visit, 1: Waiting List, 2: Post-Visit
        ...
        if st.session_state.tab_index == 0:
            ...
            with st.form("pre_visit_form", clear_on_submit=True):
                ...
                if submitted:
                    ...
                    time.sleep(2)
                    st.session_state.tab_index = 1  # Move to Waiting List
                    ...
                    with st.form("post_visit_form", clear_on_submit=True):
                        ...
                        if submitted:
                            ...
                            if push_to_github(file_path, f"Post-visit update for Patient {selected_id}"):
                                ...
                                st.warning("⚠️ GitHub push failed.")
                            st.session_state.tab_index = 0  # Go back to Pre-Visit
                            time.sleep(2)
                            st.experimental_rerun()
